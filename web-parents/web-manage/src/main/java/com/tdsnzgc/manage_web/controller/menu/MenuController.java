...
import com.tdsnzgc.manage_web.service.Permission.PermissionServiceImpl;
...
public class MenuController {
    ...
    public Result queryAllPermissionMenu(@RequestHeader(name = "token") String token) {
        List<Menu> permissions = permissionServiceImpl.queryMyPermissionMenu(token, null);
    ...
    public Result queryMyPermissionRouter(@RequestHeader(name = "token") String token) {
        List<Menu> permissions = permissionServiceImpl.queryMyPermissionMenu(token, "router");
        return new ResultUtil().setData(permissions);
    }
}
